...
def read_slot():
    return mgr.blocking_trigger('read_slot')[0].value
...
def adjust_to_tension(
        slop=10, step_size=[100, 50], high_threshold=None, drive=FP,
        adir=None):
...
def adjust_back_tension(slop=10, step_size=[50], adjusted=False):
    t = read_tension(10)
    dt = t - tension_target
    print("Tension: %s[%s], %s" % (t, dt, step_size))
    if dt > slop:
...
def move_n_slots(n=100):
    run_reels()
    for i in xrange(n):
        print("%i" % i)
        move(6)
        if (read_tension() - tension_target) > 10:
            print("\tretensioning")
            adjust()
        time.sleep(0.25)
    time.sleep(0.5)
