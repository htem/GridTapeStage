...
TreeEditor.prototype.set = function (json, name) {
  ...
  if (json instanceof Function || (json === undefined)) {
    ...
    old_root = (this.node == undefined) ? undefined : this.node.clone();
    this._setRoot(node);
    ...
    // expand children
    this.node.match_expand(old_root);
...
/**
 * Match the expansion of children from another node
 * @parame {Node} Node to match
 */
Node.prototype.match_expand = function(node) {
    if (node == undefined) {
        return;
    };

    if (node.expanded) this.expand(false);

    for (c in this.childs) {
        child = this.childs[c];
        for (oc in node.childs) {
            other = node.childs[oc];
            if (child.field == other.field) {
                child.match_expand(other);
                break;
            };
        };
    };
